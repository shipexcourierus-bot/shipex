...
window.viewShipmentTracking = async id => {
  try {
    const snap = await getDoc(doc(db,'shipments',id));
    if (snap.exists())
      window.open(`../pages/tracking.html?id=${encodeURIComponent(snap.data().trackingId)}`, '_blank');
  } catch { /**/ }
};
...
window.adminTrack = () => {
  ...
  if (id) window.open(`../pages/tracking.html?id=${encodeURIComponent(id)}`, '_blank');
